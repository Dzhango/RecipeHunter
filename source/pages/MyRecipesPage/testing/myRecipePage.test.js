...
// quick dirty fix of fetch function from stack overflow
// import fetch from 'node-fetch'
// globalThis.fetch = fetch
//import * as myModule from '../myRecipePage.js';
...
// tests for DeleteFromFavID - Jeremy
...
// tests for bindRecipeClick - Will - check if session storage and location are updated
test('bindRecipeClick: ', () => {
    
})

// tests for greyOut - Richard - needs to check classes of DOM elements 
...
<div class="recipes-container"></div>
  `;
    // populate localstorage with recipe id's
    // for (let i = 0; i < 10; i++) {
    ...
    functions.populateMyRecipe();
    let expectedGrey = localStorage.length;
...
// tests for createRecipeCards - Richard - need to check if recipe-cards populate
...
<div class="recipes-container"></div>
  `;
    ...
    localStorage.clear();
   let recipeCardList = Array.from(document.querySelectorAll('recipe-title'));
    expect(recipeCardList.length).toBe(0);

    functions.createRecipeCards(recipeData);
    recipeCardList = Array.from(document.querySelectorAll('recipe-title'));
...
// tests for addButtonOnClick - Richard - needs to check classes of DOM elements (function removed from most recent version)
// test('Calling addButtonOnClick to see if the two buttons have class hide removed and if recipe container has hide',()=>{
//     document.body.innerHTML = `
//     <div class ="addPage hide"> </div>
//     <div class="recipes-container"> </div>
//   `;
//     // require('../myRecipePage.js');
//     //checking if default value of .addPage has class hide and .recipe-container doesnt have class hide
//     let addPageElement = document.querySelector('.addPage')
//     console.log(addPageElement)
//     let recipeContainerElement = document.querySelector('.recipes-container')
//     expect(addPageElement.classList.contains("hide")).toBe(true)
//     expect(recipeContainerElement.classList.contains("hide")).toBe(false)
//     functions.addButtonOnClick();
//     //Checking if the appropriate elements has class hide 
//     expect(addPageElement.classList.contains("hide")).toBe(false)
//     expect(recipeContainerElement.classList.contains("hide")).toBe(true)

    
// })

// tests for addRecipeURL - Will
...
// tests for hashing - Will
test('Testing the hashing function to see if it return the correct hash value', () => {
    let expectedZero = functions.hashing('');
    expect(expectedZero).toBe(0);
    let hashValA = functions.hashing('a');
    let hashValB = functions.hashing('b');
    expect(hashValA - hashValB).not.toBe(0);
})

// tests for addPageCloseOnClick  - Antonia - need to check values of DOM elements (function removed from most recent version)
// test('addPageCloseOnClick', () => { 
//     document.body.innerHTML = `
//     <div class="addPage">
//     <input type="text" placeholder="Enter Url" class = "addPageText"></input>
//     <button type = "submit" class = "addPageSubmit">Enter</button>
// </div>
// <div class="recipes-container hide">s
//   `;

//     expect(document.querySelector('.addPage').classList.contains('hide')).toBe(false);
//     expect(document.querySelector('.recipes-container').classList.contains('hide')).toBe(true);
    
//     functions.addPageCloseOnClick(true);
    
//     expect(document.querySelector('.addPage').classList.contains('hide')).toBe(true);
//     expect(document.querySelector('.addPageText').value).toBe("");
//     expect(document.querySelector('.recipes-container').classList.contains('hide')).toBe(false);
//     // actualHide = (document.querySelector('hide'));
//     // expext(actualHide.length(0));
// })

// tests for addPageSubmitOnClick - Antonia - need to check classes and values of DOM elements
